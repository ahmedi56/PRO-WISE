...
const RecommendationSection = ({
    ...
    selectedComponents = [],
    onClearSelection,
    mode = 'product' // 'product' or 'components'
}) => {
    ...
    useEffect(() => {
        const fetchRecommendations = async () => {
            // In component mode, only fetch if we have a selection
            ...
            try {
                if (isComponentMode) {
                    ...
                    // Normal product-to-product recommendation
    ...
    // Hide the entire section ONLY if we're in component mode without a selection
    if (isComponentMode && !hasSelection) {
        return null;
    }

    const [discoveryFilter, setDiscoveryFilter] = useState('brand');
    ...
    // Smart Defaulting: If recommendations change and current filter is empty, switch to the one that has data.
    useEffect(() => {
                    ...
                    <div style={{ flex: 1 }}>
                        ...
                        {Array.isArray(product.matchedComponents) && product.matchedComponents.length > 0 ? (
                            <div style={{ display: 'flex', flexWrap: 'wrap', gap: '0.4rem' }}>
                                {product.matchedComponents.slice(0, 2).map((component) => (
                                    <span
