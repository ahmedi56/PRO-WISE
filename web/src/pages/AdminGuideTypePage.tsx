...
const AdminGuideTypePage: React.FC = () => {
    const { token } = useSelector((state: RootState) => state.auth);
    const [guideTypes, setGuideTypes] = useState<GuideType[]>([]);
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            if (editingId) {
                await axios.put(`${API_URL}/guidetypes/${editingId}`, formData);
            } else {
                await axios.post(`${API_URL}/guidetypes`, formData);
            }
    ...
    const handleEdit = (item: GuideType) => {
