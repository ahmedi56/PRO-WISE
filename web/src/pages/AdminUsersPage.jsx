...
const UserEditModal = ({ user, roles, companies, onClose, onSave, saving }) => {
    const [formData, setFormData] = useState({
        name: user.name || '',
        email: user.email || '',
        role: user.role?.id || user.role?.name || user.role || 'client',
...
const AdminUsersPage = () => {
    ...
    const [activeTab, setActiveTab] = useState('all'); // all, admins, clients
    ...
    const fetchUsers = async (status) => {
        ...
        // ... rest of fetch logic
        const { data } = await axios.get(`${API_URL}/users`, { ...config, params });
    ...
    const fetchInitialData = async () => {
        ...
        try {
            const [usersData, rolesData, companiesData] = await Promise.all([
                ...
                axios.get(`${API_URL}/roles`, config).catch(() => ({ data: DEFAULT_ROLES })),
                axios.get(`${API_URL}/companies`, config)
            ]);

            setUsers(usersData);
            setRoles(Array.isArray(rolesData.data) && rolesData.data.length ? rolesData.data : DEFAULT_ROLES);
    ...
    const handleAction = async (id, action) => {
        ...
        try {
            const { data } = await axios.put(`${API_URL}/users/${id}/${action}`, {}, config);
            if (data?.user) {
    ...
    const handleBulkDelete = async () => {
        ...
        setActionLoading(true);
        try {
            // Using a loop for deletion as a reliable alternative to backend bulk-delete route
    ...
    const toggleSelectAll = () => {
        if (selectedUserIds.length === finalDisplayUsers.filter(u => u.id !== currentUser?.id).length && finalDisplayUsers.length > 0) {
            setSelectedUserIds([]);
        } else {
            const selectableIds = finalDisplayUsers
    ...
    const handleUpdate = async (updatedData) => {
        ...
        try {
            ...
            const { data } = await axios.put(`${API_URL}/users/${editingUser.id}`, payload, config);
            if (data?.user) {
                updateLocalUser(data.user);
            }
            setEditingUser(null);
                    ...
                    <table className="table">
                        ...
                        <tbody>
                            ...
                            {!loading ? finalDisplayUsers.map((entry) => {
                                ...
                                return (
                                    <tr key={entry.id} style={{ backgroundColor: selectedUserIds.includes(entry.id) ? 'rgba(var(--color-primary-rgb), 0.05)' : 'transparent' }}>
                                        <td>
                                            <input 
                                                type="checkbox" 
