...
const AdminSupportPage = () => {
    ...
    const { content, loading: supportLoading, error, success } = useSelector((state) => state.support);
    
    const [selectedProductId, setSelectedProductId] = useState('');
    const [activeTab, setActiveTab] = useState('guides');
    const [showModal, setShowModal] = useState(null); // 'guide', 'step', 'video', 'pdf'
    
    // Form States
    const [guideForm, setGuideForm] = useState({ difficulty: 'medium', estimated_time: '', isPublished: false, en: { title: '', description: '' }, fr: { title: '', description: '' }, ar: { title: '', description: '' } });
    const [stepForm, setStepForm] = useState({ step_number: 1, estimated_time: '', en: { title: '', description: '' }, fr: { title: '', description: '' }, ar: { title: '', description: '' }, imageUrls: '' });
    const [videoForm, setVideoForm] = useState({ title: '', videoId: '' });
    const [pdfForm, setPdfForm] = useState({ title: '', fileUrl: '' });
    const [pdfSource, setPdfSource] = useState('local'); // Default to local for 'EZ' upload
    ...
    // Auto-setup step number when opening step modal
    useEffect(() => {
        if (showModal === 'step' && content.steps) {
            const nextStep = content.steps.length > 0 
                ? Math.max(...content.steps.map(s => s.step_number || 0)) + 1 
                : 1;
        ...
        try {
            if (pdfSource === 'local' && pdfFile) {
                ...
                if (uploadPDF.fulfilled.match(uploadResult)) {
                    finalUrl = uploadResult.payload.fileUrl;
                } else {
                    // Error is already set in state by slice
                    return;
                }
            }

            if (!finalUrl) {
                // Should not happen with validation but for safety
                return;
            }
    ...
    const handlePdfFileChange = (e) => {
        ...
        if (file) {
            setPdfFile(file);
            // EZ Upload: Auto-fill title from filename if currently empty
            if (!pdfForm.title) {
                ...
                <div className="guide-info-card">
                    <div className="guide-meta">
                        <div className="meta-item">
                            ...
                            <span className="meta-value">{content.guide.difficulty.toUpperCase()}</span>
                        </div>
                        <div className="meta-item">
                            ...
                            <div className="step-actions" style={{ display: 'flex', gap: '0.5rem' }}>
                                <button className="btn btn-outline btn-sm" onClick={() => {
                                    const fullUrl = (pdf.fileUrl && (pdf.fileUrl.startsWith('http') || pdf.fileUrl.startsWith('//'))) 
                                        ? pdf.fileUrl 
                                        : `${API_URL}${pdf.fileUrl}?token=${localStorage.getItem('token')}`;
                    ...
                    <div className="support-modal-content">
                        ...
                        <form onSubmit={handleAddGuide}>
                            <div className="support-form-grid">
                                ...
                                {/* French Translation */}
                                <div className="translation-box full-width">
                                    <span className="translation-lang-label">FR - FRANÇAIS</span>
                                    <div className="support-form-group" style={{ marginBottom: '1rem' }}>
                                        <label>Titre</label>
                                        <input className="support-input" type="text" value={guideForm.fr.title} onChange={e => setGuideForm({...guideForm, fr: {...guideForm.fr, title: e.target.value}})} />
                                    </div>
                                    <div className="support-form-group">
                                        <label>Description</label>
                                        <textarea className="support-textarea" value={guideForm.fr.description} onChange={e => setGuideForm({...guideForm, fr: {...guideForm.fr, description: e.target.value}})} />
                                    </div>
                                </div>
                            </div>
