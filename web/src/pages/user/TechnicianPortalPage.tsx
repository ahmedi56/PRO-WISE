        ...
        return (
            <PageWrapper>
                <div style={{ textAlign: 'center', padding: '4rem 2rem' }}>
                    ...
                    <p>You must be an approved technician to access this portal.</p>
                                    ...
                                    <div key={req.id} className="pw-card pw-p-5" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                                        <div style={{ flex: 1 }}>
                                            <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', marginBottom: '0.5rem' }}>
                                                ...
                                                <Badge tone={req.urgency === 'high' ? 'danger' : (req.urgency === 'medium' ? 'warning' : 'info')}>{req.urgency} urgency</Badge>
                                            </div>
                                        ...
                                        <div style={{ display: 'flex', gap: '0.5rem' }}>
                                            {req.status === 'pending' && (
                                                <Button size="sm" onClick={() => handleUpdateStatus(req.id, 'assigned')}>Accept Job</Button>
                                            )}
                                            {req.status === 'assigned' && req.technician?.id === user.id && (
                                                <Button size="sm" onClick={() => handleUpdateStatus(req.id, 'in_progress')}>Start Work</Button>
                                            )}
                                            {req.status === 'in_progress' && req.technician?.id === user.id && (
                    ...
                    <Section title="Performance">
                        <div className="pw-card pw-p-5">
                            <div style={{ marginBottom: '1.5rem' }}>
                                <div style={{ fontSize: '0.75rem', color: 'var(--color-text-muted)', textTransform: 'uppercase', letterSpacing: '0.05em', marginBottom: '0.25rem' }}>Status</div>
                                <Badge tone="success">Verified Tech</Badge>
                            </div>
                            <div style={{ marginBottom: '1.5rem' }}>
                                ...
                                <div style={{ fontSize: '2rem', fontWeight: 700, color: 'var(--color-primary)' }}>
                                    {requests.filter(r => r.status === 'completed' && r.technician?.id === user.id).length}
                                </div>
                            </div>
                            <div>
                                <div style={{ fontSize: '0.75rem', color: 'var(--color-text-muted)', textTransform: 'uppercase', letterSpacing: '0.05em' }}>Current Load</div>
                                <div style={{ fontSize: '2rem', fontWeight: 700, color: 'var(--color-secondary)' }}>
                                    {requests.filter(r => (r.status === 'assigned' || r.status === 'in_progress') && r.technician?.id === user.id).length}
                                </div>
                            </div>
                        </div>
                        <Button variant="outline" fullWidth onClick={() => navigate('/technician/profile')}>Edit Full Profile</Button>
                    </Section>
                </div>
            </div>
        </PageWrapper>
