    ...
    useEffect(() => {
        ...
        const fetchData = async () => {
            ...
            try {
                ...
                let foundVideo = (fetchedProduct.supportVideos || []).find(v => v.id === videoId || v.videoId === videoId);
                
                if (!foundVideo) {
                    // Check legacy guides
                    (fetchedProduct.guides || []).forEach(guide => {
            ...
            <div className="page" style={{ padding: '2rem 0' }}>
                ...
                <div style={{ marginTop: '2.5rem', display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: '2rem', flexWrap: 'wrap' }}>
                    <div style={{ flex: 1, minWidth: '300px' }}>
                        ...
                        <p style={{ color: 'var(--color-text-muted)', fontSize: '1.1rem', maxWidth: '800px', lineHeight: '1.6' }}>
                            Author: <span style={{ color: 'var(--color-primary)', fontWeight: 600 }}>{author}</span>
                        </p>
                    </div>
                ...
                <div style={{ marginTop: '4rem', paddingTop: '3rem', borderTop: '1px solid var(--color-border)' }}>
                    ...
                    <div className="product-grid">
                        {/* Show other videos from this product */}
                        {(product?.supportVideos || []).filter(v => v.id !== videoId && v.videoId !== videoId).map(video => (
                            <div 
                                key={video.id} 
