...
const ProfilePage: React.FC = () => {
    ...
    const handleSubmit = (event: React.FormEvent) => {
        ...
        if (activeTab === 'profile') {
            ...
            dispatch(updateCompany({ id: (user.company as any)?.id || (user.company as string), companyData: companyFormData }));
        }
    };

    const isCompanyAdmin = String(roleName).toLowerCase() === 'company_admin' || String(roleName).toLowerCase() === 'administrator' || String(roleName).toLowerCase() === 'super_admin';
                    ...
                    <div className="tab-navigation" style={{ display: 'flex', gap: '1rem', marginBottom: '2rem', borderBottom: '1px solid var(--color-border)' }}>
                        ...
                        <button 
                            ...
                            style={{ 
                                ...
                                background: 'none', 
                                border: 'none', 
                                borderBottom: activeTab === 'company' ? '2px solid var(--color-primary)' : 'none', 
                                cursor: 'pointer', 
                                fontWeight: activeTab === 'company' ? '600' : '400', 
                                color: activeTab === 'company' ? 'var(--color-primary)' : 'var(--color-text-muted)', 
                                fontFamily: 'inherit', 
                                fontSize: 'inherit' 
                            }}
                        >
                    ...
                    <div className="text-center">
                        {activeTab === 'company' && user.company && (
                            <div style={{ textAlign: 'left', marginBottom: '2rem' }}>
                                <div style={{ fontSize: '1.1rem', fontWeight: 'bold', marginBottom: '0.5rem' }}>{(user.company as any).name || 'Your Company'}</div>
                                <p style={{ color: 'var(--color-text-muted)', marginBottom: '1rem' }}>{(user.company as any).description || 'No description provided.'}</p>
                                {(user.company as any).contactInfo && <div><strong>Contact:</strong> {(user.company as any).contactInfo}</div>}
                                {(user.company as any).address && <div><strong>Address:</strong> {(user.company as any).address}</div>}
                                {(user.company as any).website && <div><strong>Website:</strong> <a href={(user.company as any).website} target="_blank" rel="noopener noreferrer">{(user.company as any).website}</a></div>}
                            </div>
                        )}
