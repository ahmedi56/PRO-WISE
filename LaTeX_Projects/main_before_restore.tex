...
We built the \textbf{PRO-WISE PLATFORM} to give users instant access to product support and guides by simply scanning a QR code. The goal is to eliminate paper manuals and make troubleshooting immediate.

\vspace{0.8cm}

Instead of wasting time looking up model numbers or navigating confusing sites, customers scan the code, pull up product specs, and find solutions for common problems right away.

\vspace{0.8cm}

We developed the system using a modern stack of web and mobile technologies, keeping it fast, easy to navigate, and ready to scale as more brands join.

\vspace{0.8cm}

This setup speeds up the help process and makes finding repairs a much smoother experience.

\vspace{0.8cm}

\textbf{Keywords:} QR Code, Product Assistance, Web Application, Mobile Application
...
\begin{figure}[H]
\centering
\includegraphics[width=0.8\textwidth]{ifix.PNG}
\caption{iFixit platform interface}
\end{figure}

\paragraph{The GE Appliances Support Portal}
GE Appliances~\cite{geappliances} offers digital manuals and technician bookings through its \textit{SmartHQ} platform. While it includes AI chat tools for home diagnostics, the system is strictly proprietary. Other hardware manufacturers cannot use this ecosystem to support their own products.

\begin{figure}[H]
\centering
\includegraphics[width=0.8\textwidth]{ge app.PNG}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.95\textwidth]{gant (2).png}
...
\begin{itemize}
    ...
    \item \textbf{Performance}: We wanted the app to feel fast, aiming for sub-2-second loads. We audited pages with Lighthouse to optimize assets, hosting images on a CDN. In MongoDB, we avoided heavy offsets by using cursor-based pagination and created compound indexes on query-heavy fields to keep response times low.
    ...
    \item \textbf{Uptime}: To handle traffic spikes, we used Nginx as a reverse proxy and load balancer. We also set up MongoDB as a three-node replica set. If one database node goes down, the system handles the failover automatically without interrupting the user.
    \item \textbf{Code Quality}: We separated business logic into Sails.js services to keep controllers thin and clean. Code format is enforced using ESLint. On the client, we went with Redux Toolkit for state management, keeping everything type-safe with strict TypeScript.
\end{itemize}
...
\begin{figure}[H]
\centering
\includegraphics[width=\textwidth,height=0.82\textheight,keepaspectratio]{images/global_class_diagram.png}
...
\begin{table}[H]
\centering
\caption{Technical Environment of PRO-WISE}
\renewcommand{\arraystretch}{1.4}
\setlength{\tabcolsep}{6pt}
\begin{tabular}{p{2.8cm}p{3.4cm}p{7.3cm}}
\toprule
\textbf{Category} & \textbf{Technology} & \textbf{Description} \\
\midrule
Frontend & React 19 / Vite / TypeScript & Dynamic web interface using reusable components, React Router, Redux Toolkit, Axios, and Recharts. \\
Mobile & React Native / Expo & Cross-platform mobile application with Expo Camera, React Navigation, Redux Toolkit, and mobile API services. \\
Backend & Node.js / Sails.js & REST API, controllers, policies, services, sockets support, and business logic management. \\
Database & MongoDB & NoSQL database for flexible and scalable data storage. \\
Authentication & JWT / OAuth 2.0 & Secure user authentication and session management. \\
Artificial Intelligence & Google Gemini API \cite{gemini} & Intelligent core using \textit{gemini-2.5-flash} (primary), \textit{gemini-1.5-flash} (fallback), and \textit{text-embedding-004} for semantic search, ranking, and recommendations. \\
Tools & Git, VS Code, Postman & Development, version control, and API testing tools. \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{table}[H]
\centering
\caption{Main technologies used in the PRO-WISE platform}
\renewcommand{\arraystretch}{1.6}
\setlength{\tabcolsep}{8pt}
\begin{tabular}{m{2.4cm}p{4cm}p{7.2cm}}
\toprule
\textbf{Logo} & \textbf{Technology} & \textbf{Role in the project} \\
\midrule
\centering\techlogo{react.png} & React & Development of the dynamic web client interface. \\
\centering\techlogo{expo (1).png} & React Native / Expo & Development and testing of the cross-platform mobile application. \\
\centering\techlogo{sails.png} & Sails.js & Backend REST API, controllers, services, and business logic. \\
\centering\techlogo{Mongodb.png} & MongoDB & NoSQL database used for persistent and flexible data storage. \\
\centering\techlogo{gemini.png} & Google Gemini API \cite{gemini} & AI services for semantic search, recommendations, and generated assistance. \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{tabular}{m{2.4cm}p{4cm}p{7.2cm}}
\toprule
\textbf{Logo} & \textbf{Tool} & \textbf{Role in the project} \\
\midrule
\centering\techlogo{Git.png} & Git & Version control and source code history management. \\
\centering\techlogo{vs.png} & Visual Studio Code & Main source code editor used during implementation. \\
\centering\techlogo{overleaf.png} & Overleaf & LaTeX writing, formatting, and report preparation support. \\
\centering\techlogo{draw.png} & Draw.io & Creation of UML diagrams and technical architecture illustrations. \\
\bottomrule
\end{tabular}
\end{table}
...
Figure~\ref{fig:seq_auth} traces the message exchange during credential validation and the password recovery flow.
...
Here is a breakdown of the communication flow:
 ...
 \begin{itemize}
     \item \textbf{Lifelines}: We modeled the interactions between the User, Login View, Controller, JWT Service, and Database.
     \item \textbf{Logic Paths}: We used conditional blocks (\textit{alt}) for login validation and optional blocks (\textit{opt}) for password recovery.
  \end{itemize}
  
  \textbf{Interaction Steps:}
  \begin{enumerate}
      \item The client application initializes and loads the login page interface.
      \item The UI displays input fields for the user's email and password.
      \item The user enters their email and password credentials.
      \item Client-side validation checks if any input fields are left blank.
      \item If validation fails, the UI throws a local error alert.
      \item If valid, the client sends a POST request with the credentials payload.
      \item The auth controller intercepts the request and queries MongoDB for the user record.
      \item MongoDB returns the matching document containing the password hash.
      \item If the user document is not found, the controller returns a 404 error.
      \item If found, the controller uses bcrypt to check the password hash.
      \item If correct, the JWT service generates a signed session token.
      \item The backend API responds with the token and user metadata.
      \item The client app saves the token, logs in the user, and redirects to the home dashboard.
  \end{enumerate}
  
  \textbf{Optional Flow: Password Reset Request}
  \begin{enumerate}
      \setcounter{enumi}{13}
      \item The user clicks the "Forgot Password" link on the interface.
      \item The client app displays the password recovery page.
      \item The user enters their registered email address.
      \item The app sends a POST request to the password recovery API route.
      \item The backend controller queries MongoDB to verify the email exists.
      \item MongoDB returns the matched user record.
      \item If the email is not registered, the server returns a 404 payload.
      \item If the email exists, the backend generates a secure recovery token.
      \item The server dispatches an email containing the recovery code.
      \item The client app displays the code verification form.
      \item The user enters the received code.
      \item The client app sends the code verification payload to the backend.
      \item The controller validates the code and checks its expiration time.
      \item If the code is invalid or expired, the UI displays an error alert.
      \item If valid, the server returns a temporary authentication state.
      \item The user types their new password.
      \item The backend hashes the new password with bcrypt and updates MongoDB.
      \item The database confirms the update and the user is redirected to login.
  \end{enumerate}
...
We designed the registration and login pages to offer clean forms and quick Google social login.
 ...
 \begin{figure}[H]
 \centering
 \includegraphics[width=0.45\textwidth]{images/web_login.png}
 \hfill
 \includegraphics[width=0.45\textwidth]{images/web_register.png}
...
Once logged in, users see a custom dashboard. Figure~\ref{fig:web_dashboard} shows the main layout.
 ...
 \begin{figure}[H]
 \centering
 \includegraphics[width=0.9\textwidth]{images/web_dashboard.png}
...
We created the profile section in Figure~\ref{fig:web_profile} to allow technicians to upload their verification files.
 ...
 \begin{figure}[H]
 \centering
 \includegraphics[width=0.9\textwidth]{images/web_profile.png}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.45\textwidth]{images/web_products_list.png}
\hfill
\includegraphics[width=0.45\textwidth]{images/web_guide_editor.png}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.45\textwidth]{images/web_submission_queue.png}
\hfill
\includegraphics[width=0.45\textwidth]{images/web_moderation.png}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.45\textwidth]{images/web_reviews.png}
\hfill
\includegraphics[width=0.45\textwidth]{images/web_service_requests.png}
\caption{Web interfaces – Product reviews and service ticket overview}
...
For the **Google Gemini API** (using \textit{gemini-1.5-flash} and \textit{text-embedding-004}):
\begin{itemize}
...
For the **Local Model Deployment** (running BAAI and a local LLM like Llama-3-8B~\cite{llama3} on cloud servers):
\begin{itemize}
...
We segment technical manuals into small text paragraphs (chunks) and compute their vector embedding using the Gemini API. The database stores these floating-point arrays alongside the raw text. To calculate similarity between the query vector ($A$) and document chunk ($B$), the backend uses the Cosine Similarity formula~\cite{manning2008}:
...
We only accept paragraphs scoring higher than $0.75$.

\subsubsection{Step-by-Step Similarity Calculation Example}

To show how our similarity pipeline operates, we trace a calculation using simple 3-dimensional vectors. Let the query vector be:
\begin{equation}
A = [0.80, 0.50, 0.30]
\end{equation}

We evaluate this query against two document chunks:
\begin{itemize}
    \item \textbf{Chunk 1} (relevant display guide): $B_1 = [0.75, 0.55, 0.25]$
    \item \textbf{Chunk 2} (irrelevant settings page): $B_2 = [0.10, 0.20, 0.90]$
\end{itemize}

First, we compute the magnitude of vector $A$:
\begin{align}
\|A\| &= \sqrt{0.80^2 + 0.50^2 + 0.30^2} \nonumber \\
&= \sqrt{0.64 + 0.25 + 0.09} = \sqrt{0.98} \approx 0.990
\end{align}

\textbf{1. Similarity calculation for Chunk 1 ($B_1$):}
- Magnitude of $B_1$:
\begin{align}
\|B_1\| &= \sqrt{0.75^2 + 0.55^2 + 0.25^2} \nonumber \\
&= \sqrt{0.5625 + 0.3025 + 0.0625} = \sqrt{0.9275} \approx 0.963
\end{align}
- Dot product $A \cdot B_1$:
\begin{align}
A \cdot B_1 &= (0.80 \times 0.75) + (0.50 \times 0.55) + (0.30 \times 0.25) \nonumber \\
&= 0.60 + 0.275 + 0.075 = 0.95
\end{align}
- Cosine Similarity:
\begin{align}
\text{Similarity}(A, B_1) &= \frac{A \cdot B_1}{\|A\| \|B_1\|} \nonumber \\
&= \frac{0.95}{0.990 \times 0.963} = \frac{0.95}{0.953} \approx 0.996
\end{align}
Since $0.996 \ge 0.75$, Chunk 1 is \textbf{accepted} as relevant RAG context.

\textbf{2. Similarity calculation for Chunk 2 ($B_2$):}
- Magnitude of $B_2$:
\begin{align}
\|B_2\| &= \sqrt{0.10^2 + 0.20^2 + 0.90^2} \nonumber \\
&= \sqrt{0.01 + 0.04 + 0.81} = \sqrt{0.86} \approx 0.927
\end{align}
- Dot product $A \cdot B_2$:
\begin{align}
A \cdot B_2 &= (0.80 \times 0.10) + (0.50 \times 0.20) + (0.30 \times 0.90) \nonumber \\
&= 0.08 + 0.10 + 0.27 = 0.45
\end{align}
- Cosine Similarity:
\begin{align}
\text{Similarity}(A, B_2) &= \frac{A \cdot B_2}{\|A\| \|B_2\|} \nonumber \\
&= \frac{0.45}{0.990 \times 0.927} = \frac{0.45}{0.918} \approx 0.490
\end{align}
Since $0.490 < 0.75$, Chunk 2 is \textbf{rejected}.
...
3. Service runs cosine similarity query against pre-computed DB embeddings. \newline
...
3. Backend checks brand compatibility schemas. \newline
...
Figure~\ref{fig:sprint4_class} models the database collections for our recommendation engine. We created schemas to store vector arrays alongside physical hardware specs to run cosine similarity queries.

\begin{figure}[H]
\centering
\includegraphics[width=0.9\textwidth]{images/sprint4class4.drawio.png}
\caption{Class diagram -- Semantic search and recommendation system}
...
\hfill
\includegraphics[width=0.45\textwidth]{images/web_semantic_search.png}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.8\textwidth]{images/usecase12.drawio.png}
...
The class diagram in Figure~\ref{fig:sprint5_class} shows the database relationships, connecting \texttt{MaintenanceRequest} to the reporting \texttt{User} and the assigned \texttt{TechnicianProfile}.
...
The dashboard screenshot in Figure~\ref{fig:web_matching} shows where admins manage technician requests and view active tickets.

\begin{figure}[H]
\centering
\includegraphics[width=0.6\textwidth]{images/web_matching.png}
\caption{Web interface -- Technician approval and request pool portal}
